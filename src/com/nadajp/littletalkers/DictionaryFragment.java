...
public class DictionaryFragment extends ListFragment implements
      OnItemSelectedListener
{
   ...
   private TextView mTextHeaderWord; // heading for the word column
         ...
         Bundle savedInstanceState)
   {
      ...
      mLanguageFilter = (Spinner) mHeaderView
            .findViewById(R.id.spinnerLanguageFilter);
      ...
      ArrayAdapter<String> dataAdapter = new ArrayAdapter<String>(
            this.getActivity(), android.R.layout.simple_spinner_item, languages);
      dataAdapter
      ...
      mTextHeaderWord = (TextView) mHeaderView.findViewById(R.id.header_word);
      mTextHeaderDate = (TextView) mHeaderView.findViewById(R.id.header_date);

      // Now do the sorting by column
      mSortColumn = Prefs.getSortColumn(getActivity());
      ...
      if (mSortColumn.equals(DbContract.Words.COLUMN_NAME_WORD))
      {
         if (mbSortAscending)
         {
            mTextHeaderWord.setCompoundDrawablesWithIntrinsicBounds(0, 0,
   ...
   public void onListItemClick(ListView l, View v, int position, long id)
   {
      ...
      intent.putExtra(Prefs.WORD_ID, id);
      startActivity(intent);
   }

   public void changeLanguage()
   {
      Cursor newValues = DbSingleton.get().getWords(mCurrentKidId, mSortColumn,
            mbSortAscending, mLanguage);
   ...
   {
      mCurrentKidId = kidId;
      Cursor newValues = DbSingleton.get().getWords(kidId, mSortColumn,
            mbSortAscending, mLanguage);
      mscAdapter.swapCursor(newValues);
      mscAdapter.notifyDataSetChanged();
    ...
    * columnIndex) { if (columnIndex ==
    * cursor.getColumnIndex(DbContract.Words.COLUMN_NAME_DATE)) { String rawdate
    * = cursor.getString(columnIndex); Log.i(DEBUG_TAG, "RAW DATE: " + rawdate);
    * String[] dateArray = rawdate.split("-"); Calendar date =
    * Calendar.getInstance(); date.set(Calendar.YEAR,
    * Integer.parseInt(dateArray[0])); date.set(Calendar.MONTH,
    * Integer.parseInt(dateArray[1])); date.set(Calendar.DAY_OF_MONTH,
    * Integer.parseInt(dateArray[2]));
    * 
    * String formatted = DateUtils.formatDateTime(view.getContext(),
    * date.getTimeInMillis(),
    * DateUtils.FORMAT_SHOW_DATE|DateUtils.FORMAT_SHOW_YEAR); Log.i(DEBUG_TAG,
    * formatted); TextView txt = (TextView) view; txt.setText(formatted); return
    * true; }
    * 
    * else if (columnIndex ==
    * cursor.getColumnIndex(DbContract.Words.COLUMN_NAME_AUDIO_FILE)) { // If
    * the column is COLUMN_NAME_AUDIO_FILE then we use custom view. String
    * mAudioFile = cursor.getString(columnIndex);
    * 
    * if (mAudioFile.isEmpty()) { view.setVisibility(View.INVISIBLE); }
    * 
    * else if (!mAudioFile.isEmpty()) { view.setFocusable(false);
    * view.setFocusableInTouchMode(false); // set the visibility of the view to
    * visible view.setVisibility(View.VISIBLE); view.setOnClickListener(new
    * MyListener(mAudioFile)); } return true; }
    * 
    ...
    * 
    * public MyListener(String audioFile) { this.mAudioFile = audioFile; }
    * 
    * @Override public void onClick(View v) { if (sbPlaying) Stop();
    * v.setPressed(true); try { if (mPlayer == null) Log.i(DEBUG_TAG,
    * "Media Player is null in onClick"); mPlayer.setDataSource(mAudioFile);
    * mPlayer.setOnCompletionListener(this); Log.i(DEBUG_TAG, "Started Playing "
    * + mAudioFile); mPlayer.prepare(); mPlayer.start(); sbPlaying = true; }
    * catch (IOException e) { Log.e(DEBUG_TAG, "Audio player start failed"); } }
    * 
    * public void onCompletion(MediaPlayer mp) { Stop(); }
    * 
    * public void Stop() { if (mPlayer == null) Log.i(DEBUG_TAG,
    * "Media Player is null in Stop()"); mPlayer.stop(); mPlayer.reset();
    * sbPlaying = false; } } }
    */
   ...
   private void sortList(String columnName)
   {
      ...
      Cursor newValues = DbSingleton.get().getWords(mCurrentKidId, columnName,
            mbSortAscending, mLanguage);
      mscAdapter.swapCursor(newValues);
      mscAdapter.setViewBinder(mViewBinder);
      mscAdapter.notifyDataSetChanged();
         ...
         builder
               ...
               .setPositiveButton(R.string.delete,
                     new DialogInterface.OnClickListener()
                     {
                        ...
                        {
                           ((DictionaryFragment) getTargetFragment())
                                 .confirmDelete();
                        }
                     })
   ...
   {
      super.onPause();
      Prefs.saveAll(getActivity(), mCurrentKidId, mLanguage, mSortColumn,
            mbSortAscending);
