...
{
   private static final String DEBUG_TAG = "DbSingleton";
    ...
    public static DbSingleton get()
    {
       if (sInstance == null)
       {
          throw new java.lang.IllegalStateException("DbSingleton has not been initialized.");
       }
       return sInstance;
    }
   ...
   public String getKidName(long kidId)
   {
      Cursor cursor = null;      
      try {
         cursor = mDb.rawQuery("SELECT " + DbContract.Kids.COLUMN_NAME_NAME + " FROM Kids WHERE " 
                                  + DbContract.Kids._ID + " = " + kidId,  null);
         cursor.moveToFirst();
   ...
   public Cursor getWords(long kidId, String sortColumn, boolean bAscending, String language)
   {
      String query;
      ...
      if (language.equals(mContext.getString(R.string.all_languages)))
      {
         query =	"SELECT _id, " + DbContract.Words.COLUMN_NAME_WORD +
			   		...
			   		" FROM Words WHERE " + DbContract.Words.COLUMN_NAME_KID + " = " + kidId +
			   		" ORDER BY " + sortColumn;	 
      }
    	else
    	{
         query =	"SELECT _id, " + DbContract.Words.COLUMN_NAME_WORD +
	   				", " + DbContract.Words.COLUMN_NAME_DATE + 
	   				", " + DbContract.Words.COLUMN_NAME_AUDIO_FILE +
	   				" FROM Words WHERE " + DbContract.Words.COLUMN_NAME_KID + " = " + kidId + " AND " + 
	   				DbContract.Words.COLUMN_NAME_LANGUAGE + " = '" + language + "' ORDER BY " + sortColumn;	  
    	}
    	if (bAscending)
    	{
    		query += " ASC";
   ...
   public Cursor getWordHistory(long kidId, long wordId)
   {
      ...
      query =  "SELECT _id, " + DbContract.Words.COLUMN_NAME_WORD +
                ...
                ", " + DbContract.Words.COLUMN_NAME_AUDIO_FILE +
                " FROM Words WHERE " + DbContract.Words.COLUMN_NAME_KID + " = " + kidId + " AND " + 
                DbContract.Words.COLUMN_NAME_TRANSLATION + " = '" + translation + "' ORDER BY " +
                DbContract.Words.COLUMN_NAME_DATE + " ASC"; 
   ...
   {
      String query = "SELECT DISTINCT " + DbContract.Words.COLUMN_NAME_LANGUAGE + " FROM Words WHERE " + 
    					DbContract.Words.COLUMN_NAME_KID +
    				   " = " + kidId + " ORDER BY " + DbContract.Words.COLUMN_NAME_LANGUAGE + " ASC";
      ...
      if (cursor.moveToFirst()) 
      {
         do 
         {
            languages.add(cursor.getString(cursor.getColumnIndex(DbContract.Words.COLUMN_NAME_LANGUAGE)));
   ...
   {
      String query = "SELECT " + DbContract.Kids.COLUMN_NAME_DEFAULT_LANGUAGE + " FROM Kids WHERE name = '" 
    					+ kidName + "' ";
   ...
   public String[] getDefaults(long kidId)
   {
      String[] rtn = new String[2];
    	String query = "SELECT " + DbContract.Kids.COLUMN_NAME_DEFAULT_LANGUAGE + "," +
    					DbContract.Kids.COLUMN_NAME_DEFAULT_LOCATION + " FROM Kids WHERE _id = " +
    					kidId;
    	//Log.i(DEBUG_TAG, query);
    	Cursor cursor = null;		 
    	try {
 	      cursor = mDb.rawQuery(query, null);
 	      cursor.moveToFirst();
 	      rtn[0]=cursor.getString(cursor.getColumnIndex(DbContract.Kids.COLUMN_NAME_DEFAULT_LANGUAGE));
 	      rtn[1]=cursor.getString(cursor.getColumnIndex(DbContract.Kids.COLUMN_NAME_DEFAULT_LOCATION));
 	   } 
    	finally {
 	      if (cursor != null) 
 	      {
   ...
   public long saveKid(String name, String birthday, String location, String language, String pictureUri)
   {
   ...
   public boolean updateKid(long id, String name, String birthday, String location, String language, String pictureUri)
   {
	   // check if another kid with this name already exists
	   String query = "SELECT " + DbContract.Kids.COLUMN_NAME_NAME + " FROM Kids WHERE " + 
			   		   DbContract.Kids.COLUMN_NAME_NAME + " = '" + name
			   		  + "' AND _id != " + id;
	   
	   // if there is a different kid (different ID) with the same name, return false
	   Cursor cursor =  mDb.rawQuery(query, null);
	   if (cursor.getCount() > 0)
	   {
		   cursor.close();
   ...
   public void deleteKids(long[] ids)
   {
	   for (long id : ids)
	   {
		   ...
		   mDb.delete("Words", DbContract.Words.COLUMN_NAME_KID + " = " + id, null);
	   }
   }
   
   public String getPicturePath(long id)
   {
      String query = "SELECT " + DbContract.Kids.COLUMN_NAME_PICTURE_URI + " FROM Kids WHERE _id = " + id;
      Cursor cursor=null;      
      try {
         cursor = mDb.rawQuery(query, null);
         cursor.moveToFirst();
   ...
   public boolean saveWord(long kidId, String word, String language, long date, String location, String audioFile,
		   String translation, String towhom, String notes)
   {
	   // check if word already exists for this kid
	   String query = "SELECT word FROM Words WHERE " + DbContract.Words.COLUMN_NAME_KID + " = " + kidId
			   		  + " AND word = '" + word + "'";
	   Cursor cursor =  mDb.rawQuery(query, null);
	   if (cursor.getCount() > 0)
	   {
   ...
   public boolean updateWord(long wordId, long kidId, String word, String language, long date, String location, String audioFile,
		   String translation, String towhom, String notes)
   {
	   // check if word already exists for this kid so we don't save the same word twice
	   String query = "SELECT word FROM Words WHERE " + DbContract.Words.COLUMN_NAME_KID + " = " + kidId
